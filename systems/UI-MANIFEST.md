...
| `stat_lines` | Inline `Label value` lines | each line binds a scalar | `lines:[{label,bind,format?,suffix?}]`; `format:"hd"` reads `{remaining,max,die}`, and `suffix` appends text to a scalar value |
