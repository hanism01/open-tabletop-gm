...
| `/gm art <search\|save\|find\|list\|update\|delete\|show\|hide>` | Manage campaign art with `python3 <skill-base>/scripts/art.py`. The GM is the LLM and uses this CLI; there is no browser-based GM art UI. See the Art Script reference for the workflow and exact arguments. |
